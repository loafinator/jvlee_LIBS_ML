...
class LIBS_1D_CNN_001(nn.Module):
    def __init__(
            self, 
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(input_channels, 32, kernel_size=kernel_size, padding=padding),
            nn.ReLU(),
            nn.Conv1d(32, 128, kernel_size=kernel_size, padding=padding),
            nn.ReLU(),
            nn.Flatten()
        )

        conv_out_size = 128 * n_features
        self.fc = nn.Sequential(
            nn.Linear(conv_out_size,512),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(512, n_wavelengths)
        )
